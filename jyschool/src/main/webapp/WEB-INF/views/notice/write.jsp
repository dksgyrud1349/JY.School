...
<c:if test="${mode=='update'}">
	function deleteFile(noticeFileNum) {
		...
		let query = "referNum=${dto.referNum}&noticeFileNum=" + noticeFileNum + "&page=${page}&size=${size}";
...
<main>
	<div class="container body-container">
	    <div class="body-title">	<!-- <i>: 공지사항이미지 -->
			<h2><i class="fas fa-clipboard-list"></i> 공지사항 </h2>	<!-- 공지사항 -->
	    </div>
			...
			<form name="noticeForm" method="post" enctype="multipart/form-data">
				<table class="table table-border table-form">
					...
					<tr>
						<td>첨&nbsp;&nbsp;&nbsp;&nbsp;부</td>
						<td> 
							<input type="file" name="clientFile" class="form-control" multiple>
						</td>
					</tr>
	
					<c:if test="${mode=='update'}">			<!-- 왜쓰는지 모르겠음 -->
				...
				<table class="table">
					<tr> 
						<td align="center">
							...
							<c:if test="${mode=='update'}">
								<input type="hidden" name="referNum" value="${dto.referNum}">
								<input type="hidden" name="page" value="${page}">
							</c:if>
						</td>
					</tr>
				</table>									<!-- 2테이블) 아래 3버튼과 값입력 -->
			</form>
	    </div>
	</div>
</main>
